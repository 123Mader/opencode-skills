...
const branch = rest.includes('--branch') ? rest[rest.indexOf('--branch') + 1] : 'main';
const concurrency = Number(rest.includes('--concurrency') ? rest[rest.indexOf('--concurrency') + 1] : 0) || 6;
...
async function api(url, body, method = 'POST', retries = 4) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (res.status === 403 || res.status === 429) {
        // 限速: 等待后重试 (指数退避)
        const wait = 3000 * Math.pow(2, attempt) + Math.floor(Math.random() * 1000);
        if (attempt < retries) { await new Promise(r => setTimeout(r, wait)); continue; }
      }
      throw new Error(`API ${res.status} ${url}: ${j.message || JSON.stringify(j).slice(0, 120)}`);
    } catch (e) {
      if (attempt < retries && /fetch failed|ECONNRESET|ETIMEDOUT/.test(e.message)) {
